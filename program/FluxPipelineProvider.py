...
class FluxPipelineProvider():
    ...
    def __init__(self, model_path: str, online_mode: bool = False, use_optimized_settings: bool = True, auth_token: str = None) -> None:
        ...
        if use_optimized_settings:
            self.__configure_pipeline_optimized()
        else:
    ...
    def __configure_pipeline_optimized(self) -> None:
        """
        This method configures pipeline in a way that it (should) balance load on both GPU and CPU
        """
    ...
    def __configure_pipeline_default(self) -> None:
        self.pipeline.to(torch.float32) #float16, float32, float64
